...
def keywrods_density(link):
    urls = requests.get("https://"+link)
    soup = BeautifulSoup(urls.content,'html.parser')
    resu = []
    stopwords = ["a", "about", "above", "above", "across", "after", "afterwards", "again", "against", "all", "almost", "alone", "along", "already", "also","although",
    ...
    for i in ((soup.stripped_strings)):
        if i.__contains__('}'):
            ...
            resu.append(i)
    word = ' '.join(resu).split(' ')
    without_stopwords = [i for i in word if i not in stopwords]
    for i,j in  Counter(without_stopwords).items():
